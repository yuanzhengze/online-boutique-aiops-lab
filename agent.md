...
任何 Agent 或成员在执行 `git commit` 前，都必须先检查并更新以下两个文档：

- `docs/daily-progress.md`：记录本次工作对应日期的完成内容、遇到的问题、需要协助事项和已提交材料。
- `docs/member-contribution.md`：补充或更新相关成员在本次工作中的贡献、交付物和证据路径。

如果本次提交是由 Agent 完成，Agent 应主动把自己的改动归属到对应负责人名下，而不是只写“Agent 完成”。例如：

- 微服务相关改动归入黄开轩。
- 监控、数据采集、Grafana 邮件通知 Agent 相关改动归入陈文涛。
- 部署、项目规则、计划和统筹相关改动归入袁正泽。
- 故障注入和性能测试相关改动归入谭张锐。
- Selenium 功能测试相关改动归入傅昱翔。
- 论文复现和算法对比相关改动归入陈宇轩。

提交前检查清单：

- [ ] `docs/daily-progress.md` 已记录本次工作进展。
- [ ] `docs/member-contribution.md` 已更新对应成员贡献。
- [ ] 本次提交涉及的截图、日志、配置或结果文件路径已写入上述文档。
- [ ] 如果没有更新这两个文档，必须在提交说明中明确说明原因。
