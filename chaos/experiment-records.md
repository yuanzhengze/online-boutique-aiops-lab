...
| 指标 | 结果 | 说明 |
|------|------|------|
| JMeter 10u 错误率 | 38.65% | 购物车和结算请求返回 HTTP 500 |
| JMeter 50u 错误率 | 0% | Pod 已自动恢复 |
| 服务可用性 | 部分不可用 | 购物车功能报错，首页/商品浏览正常 |
| 恢复时间 | ~30s | K8s 自动拉起新 Pod |
...
| 指标 | 结果 | 说明 |
|------|------|------|
| 平均响应时间 | +2.5% (10u) | 159ms → 163ms，增幅有限 |
| P99 延迟 | +126% (10u) | 312ms → 706ms，尾部延迟显著增加 |
| 错误率 | 0.02% | 极低，500ms 延迟未导致超时 |
| 服务可用性 | 100% | 延迟但不中断 |

### 分析结论

- 推荐服务延迟主要影响尾部延迟（P99 +126%），平均响应时间增幅有限
- 服务仍可用，但用户体验下降
- JMeter 测试结果保存于 results/test-results/jmeter/network-delay-recommendation-*.csv
...
| 指标 | 结果 | 说明 |
|------|------|------|
| 错误率 | 0.02% | TCP 层重传机制有效，30% 丢包未导致业务失败 |
| P99 延迟 | +124% (10u) | 312ms → 699ms，丢包导致尾部延迟增加 |
| 吞吐量 | 79.4/s | 与基线 76.5/s 相近 |
| 服务可用性 | 保持 | 所有请求最终成功

### 分析结论

- 30% 丢包下错误率仅 0.02%，TCP 重传机制有效
- 丢包主要影响尾部延迟（P99 +124%），而非直接导致请求失败
- JMeter 测试结果保存于 results/test-results/jmeter/network-loss-frontend-*.csv
...
| 指标 | 结果 | 说明 |
|------|------|------|
| 平均响应时间 | 159ms (10u) | 与基线 159ms 持平 |
| P50 延迟 | 132ms | 略低于基线 182ms |
| P99 延迟 | 817ms | 基线 312ms → 817ms，+162% |
| 错误率 | 0% | 无错误 |
| 服务可用性 | 100% | 服务正常 |

### 分析结论

- CPU Stress 对平均响应时间影响小（159ms vs 基线 159ms），但对尾部延迟影响显著
- P99 从 312ms 增至 817ms（+162%），说明 CPU 压力主要影响尾部请求
- 80% CPU 负载、2 workers 足以对 P99 造成明显影响
...
| 指标 | 结果 | 说明 |
|------|------|------|
| 平均响应时间 | 150ms (10u) | 略低于基线 159ms |
| P99 延迟 | 622ms | 基线 312ms → 622ms，+99% |
| 50u P99 | 1,953ms | 中负载下 P99 增幅最高（+95%） |
| 错误率 | 0.01% | 极低 |

### 分析结论

- Memory Stress 对平均响应时间影响小，但对尾部延迟影响显著
- P99 从 312ms 增至 622ms（+99%），50 用户下 P99 增幅最高（+95%）
- 256MB 内存压力足以对中高负载场景的尾部请求造成明显影响
